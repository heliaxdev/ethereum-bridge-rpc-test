...
use eyre::WrapErr;
use crate::namada_queries::{BASE_DIR, LEDGER_ADDRESS};
...
async fn bridge_pool_test() -> eyre::Result<()> {
    let transfers = QueryExecutor::signed_bridge_pool_transfers()
        .ledger_address(std::env::var("LEDGER").unwrap_or_else(|_| LEDGER_ADDRESS.into()))
        .base_dir(std::env::var("BASE_DIR").unwrap_or_else(|_| BASE_DIR.into()))
        .execute_query()?;
    let relayer = transfers
        ...
        .sender
        .clone();
    let mut proof = QueryExecutor::bridge_pool_proof()
        .ledger_address(std::env::var("LEDGER").unwrap_or_else(|_| LEDGER_ADDRESS.into()))
        .base_dir(std::env::var("BASE_DIR").unwrap_or_else(|_| BASE_DIR.into()))
        .hashes(vec![transfers.bridge_pool_contents.keys().cloned().collect())
        .with_relayer(relayer)
        .execute_query()?;

    proof.validator_set_args.nonce = 0.into();
    ...
    relay_bp_proof(Arc::clone(&client), proof).await
}

/// Relay a bridge pool proof to Ethereum.
async fn relay_bp_proof(client: Arc<Provider<Http>>, relay_proof: RelayProof) -> eyre::Result<()> {
